...
fn test_debounce_timing() {
    ...
    assert!(
        debounce_ms >= 5000,
        "Debounce should be at least 5 seconds for goals"
    );
}
...
fn test_capture_region_validation() {
    ...
    assert!(
        region[0] + region[2] <= 10000,
        "Region should be within reasonable screen bounds"
    );
    assert!(
        region[1] + region[3] <= 10000,
        "Region should be within reasonable screen bounds"
    );
}
...
fn test_benchmark_frame_count() {
    ...
    assert!(
        bench_frames >= 100,
        "Need enough frames for statistical significance"
    );
