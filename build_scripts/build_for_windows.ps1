...
# vcpkg logs dir (workflow will upload)
...
foreach ($t in $tools) {
    $cmdObj = Get-Command $t -ErrorAction SilentlyContinue
    if ($cmdObj) {
        # Use $($t) to avoid parser confusion when adding punctuation next to variable
        Write-Host "$($t): $($cmdObj.Path)"
    } else {
...
# run wrapper and capture environment
...
# --- Chocolatey best-effort installs for missing tools ---
...
# --- Build Rust release (MSVC target) --------------------------------------
