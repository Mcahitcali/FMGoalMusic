...
impl FMGoalMusicsApp {
    pub fn new(_cc: &eframe::CreationContext<'_>) -> Self {
        ...
        let team_database = match crate::teams::TeamDatabase::load() {
            Ok(db) => {
                ...
                tracing::info!(
                    "[fm-goal-musics] Team database loaded successfully from: {}",
                    db_path
                );
                Some(db)
            }
            Err(e) => {
                tracing::error!(
                    "[fm-goal-musics] ERROR: Failed to load team database: {}",
                    e
                );
                tracing::error!("[fm-goal-musics] Team selection will not be available.");
                tracing::error!(
                    "[fm-goal-musics] Expected location: {}",
                    crate::teams::TeamDatabase::database_path_display()
                );
                None
            }
        };
            ...
            Ok(config) => {
                ...
                if config.auto_check_updates {
                    ...
                    thread::spawn(move || {
                        ...
                        if let crate::update_checker::UpdateCheckResult::UpdateAvailable {
                            ref latest_version,
                            ..
                        } = result
                        {
                            if !crate::update_checker::should_skip_version(
                                latest_version,
                                &skipped_version,
                            ) {
                ...
                tracing::info!(
                    "✓ Loaded {} music files from config",
                    state.music_list.len()
                );
            }
            Err(e) => {
    ...
    fn save_config(&self) {
        ...
        let new_config = Config {
            ...
            music_list: state
                .music_list
                .iter()
                .map(|entry| crate::config::MusicEntry {
        ...
        if let Ok(old_config) = Config::load() {
            ...
            if old_config.capture_region != new_config.capture_region {
                changes.push(format!(
                    "capture_region: {:?} -> {:?}",
                    old_config.capture_region, new_config.capture_region
                ));
            }
            if old_config.ocr_threshold != new_config.ocr_threshold {
                changes.push(format!(
                    "ocr_threshold: {} -> {}",
                    old_config.ocr_threshold, new_config.ocr_threshold
                ));
            }
            if old_config.debounce_ms != new_config.debounce_ms {
                changes.push(format!(
                    "debounce_ms: {} -> {}",
                    old_config.debounce_ms, new_config.debounce_ms
                ));
            }
            if old_config.enable_morph_open != new_config.enable_morph_open {
                changes.push(format!(
                    "enable_morph_open: {} -> {}",
                    old_config.enable_morph_open, new_config.enable_morph_open
                ));
            }
        ...
        let ambiance_data: Option<Arc<Vec<u8>>> = if ambiance_enabled {
            if let Some(ref path) = ambiance_path {
                match std::fs::read(path) {
                    Ok(bytes) => {
                        tracing::info!(
                            "[fm-goal-musics] Preloaded ambiance sound: {} ({} bytes)",
                            path,
                            bytes.len()
                        );
                        Some(Arc::new(bytes))
                    ...
                    Err(e) => {
                        tracing::warn!(
                            "[fm-goal-musics] Warning: Failed to read ambiance file: {}",
                            e
                        );
                        None
                    }
        ...
        let handle = thread::spawn(move || {
            ...
            let ambiance_manager: Option<AudioManager> = if let Some(ref data) = ambiance_data {
                match AudioManager::from_preloaded(Arc::clone(data)) {
                    ...
                    Err(e) => {
                        tracing::warn!(
                            "[fm-goal-musics] Warning: Failed to initialize ambiance manager: {}",
                            e
                        );
                        None
                    }
            ...
            let mut capture_manager = match CaptureManager::new(
                CaptureRegion::from_array(capture_region),
                selected_monitor_index,
            ) {
                Ok(manager) => manager,
                Err(e) => {
            ...
            let mut ocr_manager =
                match OcrManager::new_with_options(ocr_threshold, enable_morph_open) {
                    Ok(manager) => manager,
                    Err(e) => {
            ...
            let team_matcher = if let Some(ref sel_team) = selected_team {
                match crate::teams::TeamDatabase::load() {
                    Ok(db) => match db.find_team(&sel_team.league, &sel_team.team_key) {
                        Some(team) => {
                            tracing::info!(
                                "[fm-goal-musics] Team matcher initialized for: {}",
                                sel_team.display_name
                            );
                            Some(crate::team_matcher::TeamMatcher::new(&team))
                        }
                        None => {
                            tracing::warn!(
                                "[fm-goal-musics] Warning: Selected team not found in database"
                            );
                            None
                        }
                    },
                    Err(e) => {
                        ...
                        tracing::error!(
                            "[fm-goal-musics] Team-specific goal detection will not work."
                        );
                        tracing::error!(
                            "[fm-goal-musics] Check that teams.json exists at: {}",
                            crate::teams::TeamDatabase::database_path_display()
                        );
                        None
                    }
...

            {
                ...
                if team_matcher.is_some() {
                    st.status_message = format!(
                        "Monitoring for goals by selected team... Playing '{}' when detected",
                        music_name
                    );
                } else {
                    st.status_message = format!(
                        "Monitoring for all goals... Playing '{}' when detected",
                        music_name
                    );
                }
            }
                    ...
                    match ocr_manager.detect_goal_with_team(&image) {
                        Ok(Some(detected_team)) => {
                            if matcher.matches(&detected_team) {
                                tracing::info!(
                                    "[fm-goal-musics] 🎯 GOAL FOR SELECTED TEAM: {}",
                                    detected_team
                                );
                                (true, selected_team.clone())
                            } else {
                                tracing::info!(
                                    "[fm-goal-musics] Goal detected for: {} (not selected team)",
                                    detected_team
                                );
                                (false, None)
                            }
                ...
                if should_play_sound && debouncer.should_trigger() {
                    ...
                    if let Some(ref ambiance) = ambiance_manager {
                        if ambiance_length_ms > 0 {
                            if let Err(e) =
                                ambiance.play_sound_with_fade_and_limit(200, ambiance_length_ms)
                            {
                    ...
                    match music_result {
                        Ok(()) => {
                            ...
                            st.status_message = format!(
                                ...
                                music_name, ambiance_msg, st.detection_count
                            );
    ...
    fn render_wizard_display_selection(&mut self, ui: &mut egui::Ui) {
        ...
        if monitor_count <= 1 {
            ...
            ui.label(format!(
                "Detected {} monitors on your system.",
                monitor_count
            ));
            ui.add_space(10.0);
            ...
            ui.label(format!(
                ...
                if state.selected_monitor_index == 0 {
                    "Primary"
                } else {
                    "Secondary"
                }
        ...
        if self.team_database.is_some() {
                ...
                egui::ComboBox::from_label("")
                    .selected_text(
                        self.selected_league
                            .as_deref()
                            .unwrap_or("-- Select League --"),
                    )
                    .show_ui(ui, |ui| {
                        if ui
                            .selectable_label(self.selected_league.is_none(), "-- None --")
                            .clicked()
                        {
                        ...
                        for league in &leagues {
                            if ui
                                .selectable_label(
                                    self.selected_league.as_ref() == Some(league),
                                    league,
                                )
                                .clicked()
                            {
                        ...
                        egui::ComboBox::from_label(" ")
                            ...
                            .show_ui(ui, |ui| {
                                for (key, team) in &teams {
                                    if ui
                                        .selectable_label(
                                            self.selected_team_key.as_ref() == Some(key),
                                            &team.display_name,
                                        )
                                        .clicked()
                                    {
            ...
            if let (Some(league), Some(team_key)) = (&self.selected_league, &self.selected_team_key)
            {
                if let Some(team) = db.find_team(league, team_key) {
                    ui.colored_label(
                        egui::Color32::GREEN,
                        format!("✓ Selected: {} ({})", team.display_name, league),
                    );
                }
            }
        ...
        egui::CollapsingHeader::new("➕ Add New Team")
            ...
            .show(ui, |ui| {
                ...
                if let Some(ref db) = self.team_database {
                    ...
                    if self.use_existing_league {
                        ...
                        egui::ComboBox::from_label("Select League")
                            ...
                            .show_ui(ui, |ui| {
                                for league in &leagues {
                                    if ui
                                        .selectable_label(&self.new_team_league == league, league)
                                        .clicked()
                                    {
                    ...
                    let variations_response =
                        egui::TextEdit::multiline(&mut self.new_team_variations)
                            .desired_rows(3)
    ...
    fn handle_add_team(&mut self) {
        ...
        if let Some(ref mut db) = self.team_database {
            ...
            let mut variations: Vec<String> = self
                .new_team_variations
                .lines()
                .map(|line| line.trim())
                            ...
                            ui.horizontal(|ui| {
                                if ui.button("📥 Download Update").clicked() {
                                    if let Err(e) = open::that(download_url) {
                                        tracing::error!(
                                            "[update-checker] Failed to open browser: {}",
                                            e
                                        );
                                    }
                                    close_modal = true;
        ...
        egui::CentralPanel::default().show(ctx, |ui| {
            ...
            theme::section_frame().show(ui, |ui| {
                ui.horizontal(|ui| {
                    ...
                    let is_active = detection_count > 0
                        || status_message.contains("Running")
                        || status_message.contains("Detecting");
                    ...
                    ui.label(
                        egui::RichText::new(format!("🎯 Detections: {}", detection_count))
                            .color(theme::GOAL_GOLD),
                    );
            ...
            if let Some(sel) = &mut self.region_selector {
                ...
                if !sel.initialized {
                    if self.capture_delay_frames > 0 {
                        tracing::info!(
                            "   Waiting for window to minimize/hide... (frames left: {})",
                    ...
                    match capture_result {
                        Err(e) => {
                            ...
                            tracing::error!(
                                "   This error occurred during screen capture for region selection"
                            );
                        ...
                        Ok(()) => {
                            tracing::info!(
                                "✓ Screenshot captured successfully for region selection"
                            );
                ...
                egui::Window::new("Select Region")
                    ...
                    .show(ctx, |ui| {
                        ...
                        if let Some(texture) = &sel.texture {
                            ...
                            sel.scale = (available.x / img_size.x)
                        ...
                        ui.horizontal(|ui| {
                            ...
                            ui.label(
                                "Click and drag over the screenshot, then release to confirm.",
                            );
                        });
                    });
...
impl RegionSelectState {
    fn capture_fullscreen(&mut self) -> Result<(), Box<dyn std::error::Error>> {
        ...
        tracing::info!(
            "Screenshot: {}x{} (physical) | Monitor: {}x{} (logical) | Scale: {}x",
            w,
            h,
            logical_w,
            logical_h,
