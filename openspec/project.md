...
- `egui`/`eframe` for the GPU-accelerated GUI, `rfd` for native dialogs, and `rdev` for global hotkeys (Cmd+1, Cmd+Shift+R).
...
- Views (in `gui/views/`) are pure and side-effect free; impure work happens in commands/executors. Use `anyhow::Context` when crossing subsystem boundaries.
- Favor `parking_lot::{Mutex,RwLock}` for shared state, `Arc<AtomicBool>` for hot paths, and avoid blocking the egui UI thread.

### Architecture Patterns
- GUI follows MVU (Model–View–Update) where `model.rs` owns state, `update.rs` maps messages to commands, and views re-render from pure data.
...
- Memory footprint should remain low enough for streamers (avoid >1 GB). Avoid long blocking operations on the UI thread to keep egui responsive at 60 FPS.
...
- **egui/eframe + rfd + rdev** – GUI toolkit, native dialogs, and global hotkeys; rely on OS accessibility permissions.
